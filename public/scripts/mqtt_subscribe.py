...
def on_message(client, userdata, message):
    ...
    if len(rows)>0:
        queryupdate=(f'UPDATE `sensor__data` SET `sensor_data_date`="{splitdata[0]}",`sensor_data_time`="{splitdata[1]}",`packet_counter`="{splitdata[3]}",`temperature`="{splitdata[4]}",`%dissolved_oxygen`="{splitdata[5]}",`mgl_dissolved_oxygen`="{splitdata[6]}" WHERE `sensor_id` = "{topic[7:]}" ')
        print(queryupdate)
    ...
    #creates a new entry for sensor data
        ...
        queryadd= (f'INSERT INTO `sensor__data`(`sensor_name`, `sensor_id`, `sensor_data_date`, `sensor_data_time`, `packet_counter`, `temperature`, `%dissolved_oxygen`, `mgl_dissolved_oxygen`) VALUES ("{topic[7:]}","{topic[7:]}","{splitdata[0]}","{splitdata[1]}","{splitdata[3]}","{splitdata[4]}","{splitdata[5]}","{splitdata[6]}")')
